...
try {
  for (const width of [1440, 390]) {
    results.push(
      ...
      for (const route of ${JSON.stringify(routes)}) {
        ...
        if (['/', '/components', '/docs', '/components/dashboard'].includes(route)) await page.screenshot({ path: 'output/playwright/overtrue/' + ${width} + '-' + (route.slice(1).replaceAll('/', '-') || 'home') + '.png', fullPage: true });
      }
  ...
  const interactions = await run(`async page => {
    ...
    await page.getByRole('textbox', { name: 'Search components', exact: true }).fill('storage meter');
    ...
    check(await page.locator('.component-tile').count() === 2, 'category filtering');
    ...
    await page.goto(${JSON.stringify(origin)} + '/components/settings-panel');
    ...
    await page.goto(${JSON.stringify(origin)} + '/components/analytics-overview');
