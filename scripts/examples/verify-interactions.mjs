...
try {
 ...
 await go('users');await page.getByPlaceholder('Search people or disciplines…').fill('Research');check(await page.locator('.scene-people-grid > section').count()===1,'filter people');await page.getByRole('link',{name:'View profile'}).click();check(await page.getByRole('heading',{name:'Ines Duarte'}).isVisible(),'open selected profile');
 ...
 await page.setViewportSize({width:390,height:844});await go('tasks');check(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1),'mobile board fits');check(!await page.locator('.workspace-sidebar').isVisible(),'desktop sidebar hidden on mobile');await page.getByRole('button',{name:'Toggle navigation',exact:true}).click();check(await page.locator('.workspace-mobile-navigation').isVisible(),'mobile workspace navigation');await page.screenshot({path:'output/playwright/workspace/mobile-navigation.png'});
 return passed;
}`,
  );
