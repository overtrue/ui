...
The Blocks library includes 368 card patterns extracted from the workspace examples, alongside the four complete interfaces. A mapped group of cards is one collection: the source keeps its demo records and interactions instead of duplicating a block for every person or project.
